...
import { Satellite, Signal, Sparkles, Activity, LayoutGrid, History, ShieldCheck } from 'lucide-react'
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 border-b border-primary/25 bg-background/85 backdrop-blur-2xl shadow-[0_4px_30px_rgba(0,0,0,0.65)]">
      <div className="flex items-center justify-between px-6 py-3">
        ...
        >
          <div className="relative flex items-center justify-center w-11 h-11 rounded-xl bg-gradient-to-br from-primary/30 via-primary/10 to-secondary/20 border border-primary/50 shadow-[0_0_25px_rgba(0,240,255,0.3)] group-hover:shadow-[0_0_35px_rgba(0,240,255,0.6)] transition-all">
            <motion.div
              animate={{ rotate: 360 }}
              transition={{ duration: 10, repeat: Infinity, ease: 'linear' }}
              className="absolute inset-0.5 rounded-xl border border-dashed border-primary/50"
            />
            <Satellite className="w-6 h-6 text-primary drop-shadow-[0_0_10px_rgba(0,240,255,0.9)]" />
          </div>
          <div>
